...
**Want to try it without risking your own repo?** This repo bundles a live two-app demo
product — [`apps/loan-webapp`](./apps/loan-webapp) + [`apps/lending-webapp`](./apps/lending-webapp)
syncing in real time over a shared data store — with a ready-made `workspace.yaml` and
contract registry. `docker compose up --build` in `apps/`, then follow the
...
- **[Demo walkthroughs](./docs/demo/README.md)** — the bundled loan-product apps:
  multi-unit init, contract checking, cross-unit goal (and the README GIF recording guide)
...
apps/                                # demo loan product: 2 webapps + shared data store, workspace.yaml,
                                     #   contracts registry — see docs/demo/README.md Part 2
