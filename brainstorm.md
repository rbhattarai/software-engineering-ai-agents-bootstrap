...
**Phase 7 (v2+).** Multi-repo tiers 2–3 from A6 (contract-check hook, federated service graph, workspace-level goal loop) — tier 1 and mono-repo support ship in v1 phases 1–3; `harness-export --client copilot|cursor|codex` (cheap: AGENTS.md core + `.github/agents/*.md` mapping, B10); web UI profile-builder (emits the install one-liner); swarm orchestration via Multica-style backend; video/NAS ingestion.
